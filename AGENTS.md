...
- **Log Output:** Logs go to stdout (`log.SetOutput(os.Stdout)` in `main`). Each `RESPONSE N` marker includes the time since the request was sent upstream, measured before the body is formatted for the log. When the upstream call or the body read fails, `logResponseError` logs `RESPONSE N (upstream error: ...)`, or `RESPONSE N (client canceled, ...)` for `context.Canceled` (only with `-responses` on), so every request has a matching entry.
